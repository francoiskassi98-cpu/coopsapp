...
export default function KpiCards({ totalPotential, totalDelivered, remaining, shipmentCount }: Props) {
  ...
  const cards = [
    { label: "Potentiel total", value: `${totalPotential.toLocaleString("fr-FR")} kg`, icon: Leaf, iconClass: "text-accent" },
    { label: "Poids livré total", value: `${totalDelivered.toLocaleString("fr-FR")} kg`, icon: Package, iconClass: "text-primary" },
    { label: "Potentiel restant", value: `${remaining.toLocaleString("fr-FR")} kg`, icon: TrendingUp, iconClass: "text-accent" },
    { label: "Taux livraison global", value: `${deliveryRate.toFixed(1)}%`, icon: BarChart3, iconClass: "text-primary" },
    { label: "Total chargements", value: shipmentCount.toString(), icon: Truck, iconClass: "text-accent" },
  ];
